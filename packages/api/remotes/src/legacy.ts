/**
 * Legacy functional API of the api-remotes package (0.1.1-rc.2 era), carried
 * verbatim so `host/apiproxy` keeps compiling while the package's own node
 * half is the rewritten cordis plugin. The only outside reference is the
 * typert lookup failure class, which the protocol renamed.
 * @module @deepseek-ai/dsh-api-remotes/legacy
 */
...
export var ApiRemoteSessionNotFound = class extends Error {};
/** Session identity whose lifecycle belongs to subagent routing. */
export var ApiRemoteSubagentSessionOwnership = class extends Error {
	sessionId;
	/**
...
*/
async export function inspectApiRemoteSession(ctx, sessionId) {
